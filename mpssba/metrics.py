...
def calculate_covariance_error(wht_t: npt.ArrayLike, exact_wht: npt.ArrayLike, n_qubits: int) -> Dict[int, float]:
    ...
    for d in distances:
        ...
        if exact_sum > 1e-12:
            err_at_t[d] = err_sum / exact_sum
        else:
            err_at_t[d] = err_sum / count if count > 0 else 0.0
            
    return err_at_t

def calculate_wht_deltas(wht_t: npt.ArrayLike, exact_wht: npt.ArrayLike, n_qubits: int, masking_type: str) -> Dict[str, float]:
    data_masks = {}
    if masking_type == "hamming":
        for h in range(1, n_qubits + 1):
            data_masks[h] = [x for x in range(2**n_qubits) if x.bit_count() == h]
        key_prefix = "h"
    elif masking_type == "correlation":
        for crl in range(1, n_qubits + 1):
            data_masks[crl] = [x for x in range(2**n_qubits) if calculate_correlation_length(x) == crl]
        key_prefix = "crl"
    elif masking_type == "corr_2body":
        for crl in range(1, n_qubits + 1):
